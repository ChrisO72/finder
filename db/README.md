...
- `schema/` contains domain modules for auth, organizations, settings, podcast feeds, episodes,
  transcript segments, semantic windows, and tags.
...
- Episodes and tags use soft deletion; reads exclude `deletedAt`.
- Feed URLs are unique per organization; episode GUIDs are unique within a feed.
