...
import { db } from "../db";
import { type InsertVideo, videos } from "../schema/videos";
...
export async function getVideoByIdForOrg(id: number, organizationId: number) {
  const [row] = await db
    ...
    .from(videos)
    .where(
      and(eq(videos.id, id), eq(videos.organizationId, organizationId), isNull(videos.deletedAt)),
    )
    .limit(1);
  return row ?? null;
}
...
export async function listVideosByOrg(organizationId: number, page: number, limit: number) {
...
export async function recentVideosByOrg(organizationId: number, limit: number = 8) {
  return await db
    .select()
    .from(videos)
    .where(
